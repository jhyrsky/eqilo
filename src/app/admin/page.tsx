...
import { Package, Users, ShoppingCart, DollarSign, ArrowUpRight, Activity, CheckCircle2 } from "lucide-react";
...
async function getDashboardStats(period: "30d" | "all") {
  const thirtyDaysAgo = new Date();
  thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);

  const [productsSnap, inStockSnap, customersSnap, recentOrdersSnap] = await Promise.all([
  ...
  const allOrdersSnap = await (
    period === "30d"
      ? adminDb.collection("orders").where("created_at", ">=", thirtyDaysAgo)
...
export default async function AdminDashboardPage({
  ...
  const period = rawPeriod === "all" ? "all" : "30d";
  const stats = await getDashboardStats(period);

  const periodLabel = period === "30d" ? "Last 30 days" : "Lifetime";

  const cards = [
    {
      title: "Total Revenue",
      value: `€${stats.totalRevenue.toLocaleString("fi-FI", { minimumFractionDigits: 2 })}`,
      label: `Gross sales · ${periodLabel}`,
      icon: DollarSign,
    ...
    },
    {
      ...
      label: `Successful checkouts · ${periodLabel}`,
      icon: ShoppingCart,
      color: "text-blue-600",
      bg: "bg-blue-50",
      href: "/admin/orders",
  ...
  return (
    <div className="space-y-10">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        {cards.map((card) => {
          const cardEl = (
            <Card key={card.title} className={`border-border/50 shadow-sm hover:shadow-md transition-all overflow-hidden group${"href" in card ? " hover:border-primary/40 cursor-pointer" : ""}`}>
          ...
          return "href" in card ? <Link key={card.title} href={card.href!}>{cardEl}</Link> : cardEl;
        })}
      </div>
